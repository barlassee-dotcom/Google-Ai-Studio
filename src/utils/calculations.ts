...
export const calculateFlow = (
  ...
  (checks || []).forEach(c => {
    if (c.effectiveDateStr >= todayStr) {
      const checkDescParts = [c.checkNo, c.company].filter(Boolean);
      const checkDesc = checkDescParts.length > 0 ? checkDescParts.join(' - ') : 'İsimsiz Çek';
      allTrans.push({
        id: c.id, date: c.effectiveDateStr, desc: `ÇEK: ${checkDesc}`,
