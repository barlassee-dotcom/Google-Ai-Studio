...
const CheckManager: React.FC<CheckManagerProps> = ({ checks, setChecks }) => {
  ...
  const resetForm = () => {
    setForm({ date: '', amount: '', checkNo: '', company: '', valor: '0' });
    setEditingId(null);
  };
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      <div className="bg-white p-6 rounded-2xl border shadow-sm h-fit">
        ...
        <div className="space-y-4">
          ...
          <div>
            <label className="text-xs font-bold text-slate-400 uppercase mb-2 block">Çek No</label>
            <input 
              className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 outline-none focus:ring-2 focus:ring-blue-500"
          ...
          <div>
            <label className="text-xs font-bold text-slate-400 uppercase mb-2 block">Firma</label>
            <input 
              className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 outline-none focus:ring-2 focus:ring-blue-500"
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50 border-b">
                  ...
                  <th className="p-4 text-xs font-bold text-slate-500 uppercase tracking-wider">Çek No</th>
              ...
              <tbody className="divide-y divide-slate-100">
                {checks.sort((a,b) => new Date(a.effectiveDateStr).getTime() - new Date(b.effectiveDateStr).getTime()).map(c => (
                  <tr key={c.id} className={`hover:bg-slate-50 transition-colors ${editingId === c.id ? 'bg-indigo-50' : ''}`}>
                    ...
                    <td className="p-4 text-sm font-medium text-slate-700">{c.checkNo}</td>
                    <td className="p-4 text-sm font-medium text-slate-700">{c.company}</td>
                ...
                {checks.length === 0 && (
                  <tr>
                    <td colSpan={7} className="p-12 text-center text-slate-400 italic">Kayıtlı çek bulunmuyor.</td>
                  </tr>
                )}
              </tbody>
            </table>
